...
# dataprep
dataprep <- function(file, outputloc) {
...
startpick <- function(data = micematrix , weight = 90){ 
  ...
  startdate <- c()
  for (row in seq_along(1:nrow(micematrix))) {
  startdate[row] <- colnames(micematrix)[which(micematrix[row, ] >= 90,
                                          arr.ind = TRUE)[1]]
...
exprun <- function(start = startdate, curdate = Sys.Date()) {
...
growth <- function(matrix, start) {
  ...
  miceDT <- as.data.table(micematrix)
  startweight <- c()
  endweight <- c()
  
  #grab the tumour size on the start day
  for (n in seq_along(1 : length(startdate))) {
    startweight[n] <- miceDT[n, as.character(startdate[n]), with = F]
  }
