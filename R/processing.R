...
#' expected format is: 1. Cage, 2. Treatment, 3. Mice ID, 4. Date of death in
#' format day/month/Year.
#' @param frame The first three columns of measures or volumes. columns:
#' 1. Cage, 2. Treatment, 3. Mouse ID.
#'
...
#'
#' This function applies the Cox proportional hazards regression model and
#' Kaplan Meier Log Rank statistical test comparing all treatments to the each
#' other and returns a table.
#'
#'
...
#' @return a data.table containing the coxph calculated coefficients.
#' @import survival
#' @import survminer
#' @importFrom stats coef
#' @importFrom utils combn
#' @export
survstats <- function(survivaldat = survframe) {
  # Get every unique combination of treatments, so we can perform a coxph
  # calculation on the seperate treatments instead of all of them grouped.
  perms <- combn(x = survivaldat[, unique(Treatment)],
                 m = 2,
                 simplify = TRUE)
  perms <- as.data.table(t(perms))
  # subset for each permutation and calculate the coxph, this returns a list
  # from which we can substract the coefficients.
  coxstats <- mapply(FUN = function(x,y){
    sub <- survivaldat[Treatment %in% c(eval(x), eval(y)), ]
    coxcalc <- coxph(Surv(survtime, binarysurvi) ~ Treatment, sub)
    coxcoef <- coef(summary(coxcalc))
  },
  x = perms$V1, y = perms$V2,
  SIMPLIFY = F)
  # and bind those coefficients with the permutations we selected earlier.
  coefmat <- matrix(data = unlist(coxstats,
                                  use.names = F),
                    nrow = nrow(perms),
                    ncol = 5,
                    byrow = T)

  # calculate the Kaplan meier Log rank test in a similar fashion.
  LRstats <- mapply(FUN = function(x,y){
    sub <- survivaldat[Treatment %in% c(eval(x), eval(y)), ]
    survfit <- survminer::surv_fit(Surv(survtime, binarysurvi) ~ Treatment,
                                   data = sub)
    LRcoef <- surv_pvalue(fit = survfit,
                          data = sub,
                          method = "survdiff")
  },
  x = perms$V1, y = perms$V2,
  ...
  LRmat <- matrix(data = unlist(LRstats,
                                use.names = F),
                  nrow = nrow(perms),
                  ncol = 4,
                  byrow = T)

  perms <- cbind(perms, coefmat, LRmat[,c(2,4)])
  colnames(perms) <- c("Treatment1",
                       "Treatment2",
                       "coef",
                       "exp(coef)",
                       "se(coef)",
